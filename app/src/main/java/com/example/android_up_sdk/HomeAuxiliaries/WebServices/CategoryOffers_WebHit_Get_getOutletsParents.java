...
public class CategoryOffers_WebHit_Get_getOutletsParents {
    private AsyncHttpClient mClient = new AsyncHttpClient();
    public static ResponseModel responseObject;
    public Context mContext;

    public void requestCategoryOffers(Context _context, final IWebCallback iWebCallback, int _page, int _categoryId, int _popularCategoryId,
                                      String _collectionId, String _playlisId, String _sortBy, String _genderType, double _lat,
                                      double _lng, boolean _isgenderRequired, String _strSearchTags, int _interestId) {

        this.mContext = _context;
        String myUrl = AppConfig.getInstance().getBaseUrlApiMobile() + ApiMethod.GET.getOutletsParents;
        RequestParams params = new RequestParams();
        params.put("page", _page);
        if (_sortBy.equalsIgnoreCase(AppConstt.DEFAULT_VALUES.SORT_BY_ALPHABETICALLY)) {
        ...
        if (_popularCategoryId > 0) {
            params.put("popular_category_id", _popularCategoryId);
        }

        if (_interestId > 0) {
            params.put("interest_tag_id", _interestId);
        }

        if (_collectionId.length() > 0) {
            params.put("collection_id", _collectionId);
        }

        if (_playlisId.length() > 0) {
            params.put("playlist_id", _playlisId);
        }
        ...
        if (_strSearchTags.length() > 0) {
            params.put("search", _strSearchTags);
        }
